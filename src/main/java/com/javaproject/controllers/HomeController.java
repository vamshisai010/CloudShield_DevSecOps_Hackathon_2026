...
public class HomeController {
    ...
    public String getBoardgameDetail(@PathVariable Long id, Model model) {
        model.addAttribute("boardgame", getBoardgameOr404(id));
        return "boardgame";
    }

    @GetMapping("/boardgames/{id}/reviews")
    ...
    @GetMapping("/secured/boardgames/{id}/addReview")
    ...
    // edit the review
    @GetMapping("/boardgames/{gameId}/reviews/{id}")
    public String editReview(@PathVariable Long gameId, @PathVariable Long id, Model model) {
        Review review = da.getReview(id);
        if (review == null || !review.getGameId().equals(gameId)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Review not found");
        }
        model.addAttribute("review", review);
        model.addAttribute("boardgame", getBoardgameOr404(gameId));
        return "secured/addReview";
    }
    ...
    public String reviewAdded(@ModelAttribute Review review) {
        ...
        System.out.println("return value is: " + returnValue);
        return "redirect:/boardgames/" + review.getGameId() + "/reviews";
    }
    ...
    public String deleteReview(@PathVariable Long id) {
        ...
        System.out.println("return value is: " + returnValue);
        return "redirect:/boardgames/" + gameId + "/reviews";
    }
    ...
    @GetMapping("/secured/play")
    public String goToPlayHub() {
        return "secured/play-hub";
    }
    ...
    private BoardGame getBoardgameOr404(Long id) {
